...
class _PlantioListViewState extends State<PlantioListView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<PlantioViewModel>(context, listen: false).fetchByLavoura(widget.lavouraId);
    });
  }

  void _showDetailsDialog(BuildContext pageContext, PlantioModel plantio) {
    ...
    Widget buildSementeDetail(int sementeID) {
      return FutureBuilder<SementeModel?>(
        future: sementeVM.getID(sementeID),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return buildDetailItem(Icons.grass, 'Semente', 'Carregando...');
          } else if (snapshot.hasError || snapshot.data == null) {
            return buildDetailItem(Icons.grass, 'Semente', 'Não encontrada');
          } else {
            return buildDetailItem(Icons.grass, 'Semente', snapshot.data!.nome);
          }
        },
      );
    }
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          title: Text('Detalhes do Plantio', style: TextStyle(color: titleColor)),
          content: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                buildDetailItem(Icons.description, 'Descrição', plantio.descricao),
                buildDetailItem(Icons.map, 'Área Plantada', "${plantio.areaPlantada.toStringAsFixed(2)} ha"),
                buildDetailItem(Icons.calendar_today, 'Data e Hora', formatarDataHora(plantio.dataHora)),
                buildSementeDetail(plantio.sementeID),
              ],
            ),
          ),
          actions: [
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        color: primaryColor,
        child: plantioVM.isLoading && lista.isEmpty
                      ...
                      return Dismissible(
                        ...
                        onDismissed: (_) {
                          if (item.id != null) {
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(content: const Text('Plantio excluído.'), backgroundColor: errorColor),
                            );
                          }
                        },
                        child: Card(
                              ...
                              child: Row(
                                children: [
                                  CircleAvatar(
                                    ...
                                    child: Icon(Icons.agriculture, color: iconColor),
                                  ),
        ...
        children: [
          ...
          FloatingActionButton(
            ...
            onPressed: () => Navigator.push(
              context,
              MaterialPageRoute(
                builder: (_) => PlantioFormView(lavouraId: widget.lavouraId),
              ),
            ),
