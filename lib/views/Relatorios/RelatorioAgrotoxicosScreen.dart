...
class RelatorioAgrotoxicosScreen extends StatefulWidget {
  const RelatorioAgrotoxicosScreen({Key? key}) : super(key: key);

  @override
  State<RelatorioAgrotoxicosScreen> createState() =>
      _RelatorioAgrotoxicosScreenState();
}

class _RelatorioAgrotoxicosScreenState
    extends State<RelatorioAgrotoxicosScreen> {
  DateTime? _dataInicio;
  DateTime? _dataFim;
  bool _isGenerating = false;
  String? _errorMessage;
  RelatorioAplicacoesResponseDto? _relatorio;
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      _relatorio = await _relatorioService.gerarRelatorioAgrotoxicos(
        usuarioId: usuarioId,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Relatório de Agrotóxicos gerado com sucesso!'),
  ...
  Future<void> _generatePDF() async {
    ...
    final periodo =
        '${_dataInicio!.day}/${_dataInicio!.month}/${_dataInicio!.year} - ${_dataFim!.day}/${_dataFim!.month}/${_dataFim!.year}';

    await PdfService.generateAgrotoxicosPDF(
  ...
  Widget _buildAplicacaoCard(AplicacaoDto aplicacao) {
    return Card(
      margin: const EdgeInsets.symmetric(vertical: 4, horizontal: 8),
      child: ListTile(
        title: Text(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoCard(
                        'Total de Registros',
                        _relatorio!.totalRegistros.toString(),
                      ),
                      if (_relatorio!.error != null)
                        _buildInfoCard('Erro', _relatorio!.error!),
                      ...
                      const SizedBox(height: 8),
                      ..._relatorio!.data.map(
                        (aplicacao) => _buildAplicacaoCard(aplicacao),
                      ),
                    ],
                  ),
                ),
