...
class RelatorioPlantiosScreen extends StatefulWidget {
  const RelatorioPlantiosScreen({Key? key}) : super(key: key);

  @override
  State<RelatorioPlantiosScreen> createState() =>
      _RelatorioPlantiosScreenState();
}

class _RelatorioPlantiosScreenState extends State<RelatorioPlantiosScreen> {
  DateTime? _dataInicio;
  DateTime? _dataFim;
  bool _isGenerating = false;
  String? _errorMessage;
  RelatorioPlantiosDto? _relatorio;
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      _relatorio = await _relatorioService.gerarRelatorioPlantios(
        usuarioId: usuarioId,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Relatório de Plantios gerado com sucesso!'),
  ...
  Future<void> _generatePDF() async {
    ...
    await PdfService.generatePlantiosPDF(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoCard(
                        'Período',
                        _relatorio!.estatisticas.periodoAnalisado,
                      ),
                      _buildInfoCard(
                        'Total de Plantios',
                        _relatorio!.estatisticas.totalPlantios.toString(),
                      ),
                      _buildInfoCard(
                        'Área Total Plantada',
                        '${_relatorio!.estatisticas.areaTotalPlantada.toStringAsFixed(2)} ha',
                      ),
                      ...
                      const SizedBox(height: 8),
                      ..._relatorio!.plantios.map(
                        (plantio) => _buildPlantioCard(plantio),
                      ),
                    ],
                  ),
                ),
