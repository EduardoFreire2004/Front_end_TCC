...
class RelatorioSementesScreen extends StatefulWidget {
  const RelatorioSementesScreen({Key? key}) : super(key: key);

  @override
  State<RelatorioSementesScreen> createState() =>
      _RelatorioSementesScreenState();
}

class _RelatorioSementesScreenState extends State<RelatorioSementesScreen> {
  DateTime? _dataInicio;
  DateTime? _dataFim;
  bool _isGenerating = false;
  String? _errorMessage;
  RelatorioEstoqueDto? _relatorio;
  ...
  Future<void> _generateReport() async {
    ...
    try {
      ...
      _relatorio = await _relatorioService.gerarRelatorioSementes(
        usuarioId: usuarioId,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Relatório de Sementes gerado com sucesso!'),
  ...
  Future<void> _generatePDF() async {
    ...
    // TODO: Implementar PDF específico para sementes
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('PDF de Sementes será implementado em breve'),
          ...
          children: [
            Text(
              ...
              style: TextStyle(
                ...
                color:
                    movimentacao.tipo == 'Entrada' ? Colors.green : Colors.red,
                fontSize: 16,
              ),
            ),
          ],
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoCard(
                        'Produtos em Estoque',
                        _relatorio!.estatisticas.produtosEmEstoque.toString(),
                      ),
                      if (_relatorio!.estatisticas.valorEstoque != null)
                        _buildInfoCard(
                          'Valor do Estoque',
                          'R\$ ${_relatorio!.estatisticas.valorEstoque!.toStringAsFixed(2)}',
                        ),
                      const SizedBox(height: 16),
                      const Text(
                        'Aplicações de Sementes:',
                        style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          color: Colors.teal,
                      ...
                      const SizedBox(height: 8),
                      ..._relatorio!.movimentacoes.map(
                        (movimentacao) => _buildMovimentacaoCard(movimentacao),
                      ),
                    ],
                  ),
                ),
